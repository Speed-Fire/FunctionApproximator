...
namespace FunctionApproximator.GraphSeries
{
	...
	/// Provides an abstract base class for series that are related to an X-axis and a Y-axis.
	/// </summary>
	public abstract class CustomXYAxisSeries : ItemsSeries, ITransposablePlotElement
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="CustomXYAxisSeries"/> class.
        /// </summary>
        protected CustomXYAxisSeries()
        {
        ...
        protected TrackerHitResult GetNearestInterpolatedPointInternal(ReadOnlySpan<DataPoint> points, ScreenPoint point)
        {
        ...
        /// <returns>A tracker hit result if a point was found.</returns>
        /// <remarks>The Text property of the result will not be set, since the formatting depends on the various series.</remarks>
        protected TrackerHitResult GetNearestInterpolatedPointInternal(ReadOnlySpan<DataPoint> points, int startIdx, ScreenPoint point)
        {
            ...
            for (int i = startIdx; i + 1 < points.Length; i++)
            {
                var p1 = points[i];
                var p2 = points[i + 1];
        ...
        /// <remarks>The Text property of the result will not be set, since the formatting depends on the various series.</remarks>
        protected TrackerHitResult GetNearestPointInternal(ReadOnlySpan<DataPoint> points, ScreenPoint point)
        {
        ...
        /// <returns>A <see cref="TrackerHitResult" /> if a point was found, <c>null</c> otherwise.</returns>
        /// <remarks>The Text property of the result will not be set, since the formatting depends on the various series.</remarks>
        protected TrackerHitResult GetNearestPointInternal(ReadOnlySpan<DataPoint> points, int startIdx, ScreenPoint point)
        {
            var spn = default(ScreenPoint);
            var dpn = default(DataPoint);
            double index = -1;
            ...
            double minimumDistance = double.MaxValue;
            int i = startIdx;
            for (var j = startIdx; j < points.Length; j++)
            {
                if (!this.IsValidPoint(points[j]))
                {
                    i++;
                    continue;
                }

                var sp = this.Transform(points[j].x, points[j].y);
        ...
        protected void InternalUpdateMaxMin(ReadOnlySpan<DataPoint> points)
        {
            if (points == null)
            {
        ...
        protected int UpdateWindowStartIndex<T>(ReadOnlySpan<T> items, Func<T, double> xgetter, double targetX, int lastIndex)
        {
        ...
        /// </returns>
        public int FindWindowStartIndex<T>(ReadOnlySpan<T> items, Func<T, double> xgetter, double targetX, int initialGuess)
        {
            int start = 0;
            int nominalEnd = items.Length - 1;
            while (nominalEnd > 0 && double.IsNaN(xgetter(items[nominalEnd])))
                nominalEnd -= 1;
            int end = nominalEnd;
            int curGuess = Math.Max(0, Math.Min(end, initialGuess));

            double GetX(ReadOnlySpan<T> initems, int index)
            {
                while (index <= nominalEnd)
                {
                    double guessX = xgetter(initems[index]);
                    if (double.IsNaN(guessX))
                        index += 1;
                    else
                        return guessX;
                }
                return xgetter(initems[nominalEnd]);
            }

            while (start < end)
            {
                double guessX = GetX(items, curGuess);
                ...
                double endX = GetX(items, end);
                double startX = GetX(items, start);
