...
namespace OxyPlot
{
    ...
    public partial class PlotModel
    {
        ...
        private void RenderSeries(IRenderContext rc, ref OxyRect rect)
        {
            ...
            var withRect = this.Series.Where(s => s.IsVisible && s.HasRenderWithRect);
            var withnoRect = this.Series.Where(s => s.IsVisible && !s.HasRenderWithRect);

			this.RenderPlotElements(withRect, rc, ref rect, (withRect, rect) => withRect.Render(rc, ref rect));
			this.RenderPlotElements(withnoRect, rc, (withnoRect) => withnoRect.Render(rc));
        }

        private void RenderPlotElements<T>(IEnumerable<T> plotElements, IRenderContext rc, ref OxyRect rect, Action<T, OxyRect> renderAction) where T: PlotElement
        {
