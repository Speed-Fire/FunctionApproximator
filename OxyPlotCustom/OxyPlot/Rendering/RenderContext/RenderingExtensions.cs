...
namespace OxyPlot
{
    ...
    public static class RenderingExtensions
    {
        ...
        */
		...
		/// <param name="dashArray">The dash array (in device independent units, 1/96 inch).</param>
		/// <param name="lineJoin">The line join.</param>
		/// <param name="rect">The line join.</param>
		/// <param name="outputBuffer">The output buffer.</param>
		/// <param name="pointsRendered">The points rendered callback.</param>
		public static void DrawReducedLine(
			this IRenderContext rc,
			...
			Action<IList<ScreenPoint>>? pointsRendered = null)
		{
			...
			ReducePoints(points, minDistSquared, outputBuffer, ref rect);
			rc.DrawLine(outputBuffer, stroke, strokeThickness, edgeRenderingMode, dashArray, lineJoin);
		...
		/// <param name="minDistSquared">The minimum line segment length (squared).</param>
		/// <param name="outputBuffer">The output buffer. Cannot be <c>null</c>.</param>
		/// <param name="rect">The output buffer. Cannot be <c>null</c>.</param>
		/// <remarks>Points that are closer than the specified distance will not be included in the output buffer.</remarks>
		private static void ReducePoints(IList<ScreenPoint> points, double minDistSquared, List<ScreenPoint> outputBuffer, ref OxyRect rect)
		{
			var n = points.Count;
			if (n == 0)
			{
            ...
            if(firstPointIndex > 0)
            {
                var sc = points[firstPointIndex - 1];
                var scf = points[firstPointIndex];

                if (sc.y != scf.y && (Math.Min(sc.y, scf.Y) < 0 || Math.Max(sc.y, scf.y) > rect.Height))
                {
                    var y = rect.Height;
                    var x = ComputeX(in sc, in scf, ref y);

                    outputBuffer[0] = new ScreenPoint(x, y);
                }
            }

            if(lastPointIndex < n - 1)
            {
				var sc = points[lastPointIndex];
				var scf = points[lastPointIndex + 1];
				...
				if (sc.y != scf.y && (Math.Min(sc.y, scf.Y) < 0 || Math.Max(sc.y, scf.y) > rect.Height))
				{
					...
					outputBuffer.Add(new ScreenPoint(x, y));
				}
			}
		}

        private static double ComputeX(in ScreenPoint p1, in ScreenPoint p2, ref double y0)
        {
            y0 = Math.Min(p1.y, p2.y) < y0 && y0 < Math.Max(p1.y, p2.y) ? y0 : 0;
