...
namespace OxyPlot
{
	...
	public class CanonicalSpline : IInterpolationAlgorithm 
    {
        ...
        {
            return CreateSpline(CollectionsMarshal.AsSpan(points),
                this.Tension, null, isClosed, tolerance);
        }
		...
		/// <param name="tolerance">The tolerance.</param>
		/// <returns>A list of data points.</returns>
		public List<DataPoint> CreateSpline(ReadOnlySpan<DataPoint> points, bool isClosed, double tolerance)
		{
		...
		/// <param name="tolerance">The tolerance.</param>
		/// <returns>A list of data points.</returns>
		internal static List<DataPoint> CreateSpline(ReadOnlySpan<DataPoint> points, double tension, IList<double> tensions, bool isClosed, double tolerance)
        {
            var screenPoints = new List<ScreenPoint>(points.Length);
            for(int i = 0; i < points.Length; i++)
            {
                screenPoints.Add(new(points[i].X, points[i].Y));
            }

            //var screenPoints = points.Select(p => new ScreenPoint(p.X, p.Y)).ToList();
            var interpolatedScreenPoints = CreateSpline(screenPoints,
                tension, tensions, isClosed, tolerance);
